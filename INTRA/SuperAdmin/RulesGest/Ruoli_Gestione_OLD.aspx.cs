...
{
    public partial class Ruoli_Gestione : System.Web.UI.Page
    {
        ...
        private void getUserPrivileges()
        {
            try
            {
                if (Ruoli_Combobox.SelectedItem != null)
                {
                    string strQuery = "EXEC PRT_Privilege_Rules_getRolePrivileges '" + Ruoli_Combobox.SelectedItem.Text + "'";
        ...
        private void UpdatePermessi()
        {
            try
            {
                for (int i = 0; i < grdPrivileges.Rows.Count; i++)
                {
                    ...
                    int intFormId = int.Parse(((HiddenField)grdPrivileges.Rows[i].FindControl("hdnFormId")).Value);
                    string hdnRolesId = Ruoli_Combobox.SelectedItem.ToString();
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        strSQLQuery += " '" + Ruoli_Combobox.SelectedItem.Text + "',";
                        strSQLQuery += " '" + isAdd + "',";
        ...
        protected void Ruoli_Combobox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ControlRolePrivileges();
            getUserPrivileges();
            FireJavascriptEvents();
        ...
        protected void Aggiorna_Utenti_Callback_Callback(object source, DevExpress.Web.CallbackEventArgs e)
        {
            string RuoloID = e.Parameter;
            try
            {
                Sql4Helper objSqlHelper = new Sql4Helper();
                SqlParameter[] objParams = new SqlParameter[1];
                objParams[0] = new SqlParameter("@Rules_id_fk", RuoloID);
