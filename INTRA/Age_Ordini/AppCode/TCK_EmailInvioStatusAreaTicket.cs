...
namespace INTRA.Age_Ordini.AppCode
{
    public class TCK_EmailInvioStatusAreaTicket
    {
        ...
        public void EmailInvioStatusAreaTicket_Insert(TCK_EmailInvioStatusAreaTicket Obj)
        {
            ...
            objSqlHelper.ExecuteNonQueryForNews("U_EmailInvioStatusAreaTicket_Insert", objParams);
        }
        ...
        public TCK_EmailInvioStatusAreaTicket EmailInvioStatusAreaTicket__Get(int IdRow)
        {
            ...
            using (SqlDataReader reader = objSqlHelper.ExecuteReader("U_EmailInvioStatusAreaTicket__Get", objParams))
            {
                while (reader.Read())
                {
        ...
        {
            string conString = (string)ConfigurationManager.ConnectionStrings["info4portaleConnectionString"].ToString();
            SqlConnection con = new SqlConnection(conString);
            con.Open();
            string sql = @"DELETE FROM U_EmailInvioStatusAreaTicket where IdRow = " + IdRow;
