...
{
    public class CartItem_wish
    {
        private string _NomeContattoRM;

        public CartItem_wish()
        {
        }

        public CartItem_wish(string MenuItemID, string ItemName, int qtasel, int Quantity,
            ...
            decimal QuotaRidotta, string ScontoApplicato, decimal PercentualeSconto, string TokenAttributi, bool EnableEditQta, bool GiorniRichiestiFlag)
        {
            ...
            _NomeContattoRM = NomeContattoRM;
        ...
        public int qtasel { get; set; }
        public int Quantity { get; set; }
        public decimal ItemPrice { get; set; }
        public decimal LineValue => Quantity * ItemPrice;
    ...
    public class ShoppingCart_wish
    {
        ...
        public ShoppingCart_wish()
        {
            if (Items == null)
            {
                Items = new List<CartItem_wish>();
            }

            // _salesTaxPercent = Convert.ToDecimal(ConfigurationManager.AppSettings("SalesTax"))
            // _DeliveryCharge = Convert.ToDecimal(ConfigurationManager.AppSettings("DeliveryCharge"))
            // _MinimoAcq = Convert.ToDecimal(ConfigurationManager.AppSettings("MinimoAcq"))
            // _AttivaSpeseTrasporto = Convert.ToDecimal(ConfigurationManager.AppSettings("AttivaSpeseTrasporto"))
        ...
        public List<CartItem_wish> Items { get; }
        ...
        public decimal SubTotal
        {
            ...
            {
                decimal tot = 0;
                foreach (CartItem_wish item in Items)
                {
                    tot += item.LineValue;
                }
        ...
        public void Insert(string MenuItemID, string ItemName, int qtasel, int Quantity,
            decimal ItemPrice, int IdContattoRM, string NomeContattoRM, decimal Quota,
            decimal QuotaRidotta, string ScontoApplicato, decimal PercentualeSconto, string TokenAttributi, bool EnableEditQta, string Stagione, bool GiorniRichiestiFlag, int CategoryID)
        {
            ...
            if (idx == -1)
            {
                ...
                CartItem_wish NewItem = new CartItem_wish
                {
                ...
                NewItem.IdContattoRM = IdContattoRM;
                NewItem.NomeContattoRM = NomeContattoRM;
                NewItem.Quota = Quota;
                NewItem.QuotaRidotta = QuotaRidotta;
                NewItem.ScontoApplicato = ScontoApplicato;
                NewItem.PercentualeSconto = PercentualeSconto;
                NewItem.TokenAttributi = TokenAttributi;
                NewItem.EnableEditQta = EnableEditQta;
                NewItem.Stagione = Stagione;
                NewItem.GiorniRichiestiFlag = GiorniRichiestiFlag;
        ...
        public void Update(string MenuItemID, int qtasel, string itemName, int Quantity, decimal prezzo, int IdContattoRM, bool EnableEditQta)
        {
