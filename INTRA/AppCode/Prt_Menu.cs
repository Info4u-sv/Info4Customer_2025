...
namespace INTRA.AppCode
{
    public class PRT_Menu
    {
        ...
        public static string GetCurretMenuTitle()
        {
            ...
            List<PRT_Menu> _GetPrt_Menu_det = new List<PRT_Menu>();
            _GetPrt_Menu_det = GetPrt_Menu_det();
            List<PRT_Menu> ReturnRow = _GetPrt_Menu_det.Where(item => item.Url == path + ".aspx").ToList();
        ...
        public static List<PRT_Menu> GetPrt_Menu_det()
        {
            DataTable dt = new DataTable();
            //string CacheTable = "CacheGetPrt_Menu_det";
            List<PRT_Menu> LocalList = new List<PRT_Menu>();
            //if (!CacheHelper.ItemExists(CacheTable))
            if (1 == 1)
            {

                string SqlString = "SELECT * FROM [PRT_Menus] ORDER BY DisplayOder";
                using (SqlConnection myConnection = new SqlConnection())
                {
                    ...
                    if (myReader.HasRows)
                    {
                        while (myReader.Read())
                        {
                            PRT_Menu _objLocal = new PRT_Menu();
        ...
        public static PRT_Menu GetPrt_Menu_detByNomeCartella(string NomeCartella)
        {
            List<PRT_Menu> _objListFilterd = new List<PRT_Menu>();
            _objListFilterd = PRT_Menu.GetPrt_Menu_det().Where(x => x.Title == NomeCartella).ToList();
        ...
        public static string GetDefaultPage()
        {
            ...
            List<PRT_Menu> LocalListAppoggio = new List<PRT_Menu>();
