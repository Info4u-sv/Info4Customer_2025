...
{
    public class PRT_FunzioniGenerali_23
    {
        public PRT_FunzioniGenerali_23()
        {
            //
        ...
        {
            return t != null && IsNullable(t) ? !t.IsValueType ? t : Nullable.GetUnderlyingType(t) : t;
        }
        ...
        {
            _ = ConfigurationManager.ConnectionStrings["info4portaleConnectionString"].ConnectionString;
        ...
        {
            return string.IsNullOrWhiteSpace(text) ? null : text.Split(separator).Select(t => t.Trim()).ToArray();
        }
...
#pragma warning restore CS0219 // La variabile 'retVal' è assegnata, ma il suo valore non viene mai usato
        ...
        /*
            Funzione aggiunta in data 22/03/2023
            Andrea
            
            La procedura converte stringhe con elementi html con una stringa normalizzata
        ...
        public static string HTMLToText(string HTMLCode)
        {
            ...
            // Remove HEAD tag  
            HTMLCode = Regex.Replace(HTMLCode, "<head.*?</head>", string.Empty, RegexOptions.IgnoreCase | RegexOptions.Singleline);
            // Remove any JavaScript  
            HTMLCode = Regex.Replace(HTMLCode, "<script.*?</script>", string.Empty, RegexOptions.IgnoreCase | RegexOptions.Singleline);
